...
func TestSingleEdgeBetweenVertices(t *testing.T) {
	...
	v1 := g.AddVertex();
	v2 := g.AddVertex();
	edge := g.ConnetVertices(v1, v2);
	edge.Weight = 2;

	//sneacking a second test that makes sure params are unordered
	edge1 := g.ConnectVertices(v2, v1);

	if edge1.Weight != 2 {
		t.Errorf("Didn't correctly return the sam edge: %d", edge1.Weight)
	}
}
...
func TestDfsTraversal(t *testing.T) {
	...
	if i != 99 {
		t.Errorf("Didn't visit all vertices: %i", i)
	}
}
...
func TestFindConnectedComponents(t *testing.T) {
	...
	for i := 0; i < 99; i++ {
		...
		if disjoint.Find(result[v1.Identifier()]) != disjoint.Find(result[v2.Identifier()]) {
			t.Errorf("Two node were not in the same connected component despite being connected");
		}
	}
}
