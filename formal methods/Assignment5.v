...
Theorem and_example' :
  (beautiful 3) /\ (beautiful 6).
Proof.
  apply conj.
  apply b_3.
  apply (b_sum 3 3 b_3 b_3).
Qed.
Definition and_example'' :=
  conj (beautiful 3) (beautiful 6) (b_3) (b_sum 3 3 (b_3) (b_3)) .
Check and_example''.
...
(** We've already seen several places where analogous structures
    appcan be found in Coq's computational ([Type]) and logical ([Prop])
...
(** Logical falsehood can be represented in Coq as an inductively
    defined roposition with no constructors. *)
...
(* FILL IN HERE *)
(** [if P = true then ~(true /\ ~true)
     if P = false then ~(false /\ ~false] *)
...
(** In English, what does the proposition 
    ...
    mean? *)
(*在自然数中，存在一个数，他的后继是美丽数。*)
(* FILL IN HERE *)

(** Complete the definition of the following proof object: *)

Definition p : ex nat (fun n => beautiful (S n)) :=
    ex_intro _ (fun n => beautiful (S n)) (2) (b_sum (3) (0) b_3 b_0).
(** [] *)
...
(* ######################################################### *)
(** ** Digression(离题): More Facts about [<=] and [<] *)
