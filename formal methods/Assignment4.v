...
Definition b_n_0 : forall n, beautiful n -> beautiful (0+n) := 
  fun n => fun H : beautiful n =>
    b_sum 0 n b_0 H.
(*
...
Proof.
  ...
  Case "n = 0".     reflexivity.
  Case "n = S n'".  simpl. rewrite -> IHn'. reflexivity.  Qed.
Theorem gorgeous__beautiful_m_n : forall n m, 
  gorgeous (n + m) -> beautiful (n + m).
Proof.
  intros.
  induction H.
  apply b_0.
...
(* FILL IN HERE *)
不能这样用，induction会分情况讨论，但是它分的情况和ev的情况并不匹配*)
(** [] *)
     ...
     foo'_ind :
        forall (X : Type) (P : foo' X -> Prop),
              (forall (l : list X) (f : foo' X),
                    P f -> P (C1 X l f)) -> 
                       P (C2 X) ->
             forall f : foo' X, forall f1 : foo' X, P f1
*)
...
(** Suppose we make the following inductive definition:
   ...
   foo_ind
        : forall (X Y : Set) (P : foo X Y -> Prop),   
          (forall x : X, P (foo_1 X Y x)) ->
          (forall y : Y, P (foo_2 X Y y)) ->
          (forall f1 : foo'' X Y, P f1 -> P (foo_3 X Y f1)) ->
       forall f2 : foo'' X Y, P f2
