...
The DurableObjectHandler class is given an object to durable 
objects persisted to the same directory. Whenwver a savepoint
is reached in the application, the handler's save() method may 
be used to store the objects state to disk. The application or
machine is permitted to fail at any point. If the failure occurs
mid-write, the corruption will be detected by the DurableObjectHandler
constructor and the previously stored state will by loaded.

This implementation does not reliably protect against on-disk 
corruption. That is to say, if the most-recently saved file is 
modified after it is successfully written to the disk, such as 
through hardware or softwart errors, the saved state will be
permanently lost. As this implementation provides now way to
distinguish between files that failed to write and files that
were corrupted-on-disk, the implementation will assume the initial
write failed and will return the previously saved state(assuming 
it isn't similarly corrupted). Consequently, there is a small but
real chance that an application may save its state with this
implementation, make promises to external entities, crash, and 
reneg on those promises after recovery. The likelyhood is, 
of course, very very low so this implementation should be suitable
for most 'good' reliabilty systems. Just don't implement a life-
support system based on this code...

Design Approach:
* Toggle writes between two different files
* Include a monotonically incremeting serial number in each write to 
  allow determination of the most recent version
* md5sum the entire content of the data to be written and prefix the
  write with the digest
...
# This module is primarily concerned with flushing data to the disk.
# The default os.fsync goes a bit beyond that so os.fdatasync is used
# instead if it's available. Failing that, fcntl.fcntl(fd, fcntl.F_
# FULLSYNC) is attempted (OSX equivalent). Failing that, os.fsync is
# used (Windows).
...
class DurableObjectHandler(object):
	def __init__(self, dirname, object_id):
		'''
		Throw UnrecoverableFailure if both files are corrupted
		'''
