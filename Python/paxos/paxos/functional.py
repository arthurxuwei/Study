'''
This module provides a fully functional Paxos implementation
based on a simple heartbeating mechanism.
'''
...
class HeartbeatMessenger (practical.Messenger):
	...
	def schedule(self, msec_delay, func_obj):
		'''
		While leadership is held, this method is called
		by pulse() to schedule the next call to pulse().
		If this method is not overridden appropriately,
		subclasses must use the on_leadership_acquired()
		/on_leadership_lost() callbacks to ensure that
		pulse() is called every hb_period while leadership
		is held.
		'''
	...
	def on_leadership_change(self, prev_leader_uid, new_leader_uid):
		...
		Either UID may be None.
		'''
		
class HeartbeatNode(practical.Node):
	'''
	This class implements a Paxos node that provides a resonable
	assurance of progress through a simple heartbeating mechanism
	that is used to detect leader failure and initiate leadership
	acquisition.
	If one or mor heartbeat messages are not received within the 
	'liveness_window', leadership acquisition will be attempted 
	by sending out phase 1a, Prepare messages. If a quorum of 
	replies acknowledging leadership is received,  the node has 
	successfully gained leadership and will begin sending out 
	heartbeat messages. If a quorum is not received, the node will
	continually send a prepare every 'liveness_window' until either
	a quorum is established or a heartbeat with a proposal number 
	greater than its own is received. The units for hb_period and
	liveness_window is seconds and floating point values may be
	used for sub-second precision.
	
	Leadership loss is detected by way of receiving a heartbeat 
	message	from a proposer with a higher proposal number(which 
	must be obtained through a successful phase 1).Or by receiving 
	a quorum of NACK responses to Accept! messages.
	
	This process does not modify the basic Paxos algorithm in any 
	way, it merely seeks to ensure recovery from failures in leadership.
	Consequently, the basic paxos safety mechanisms remain intact.
	'''
	...
	def poll_liveness(self):
		'''
		Should be called every liveness_window. This method checks to
		see if the current leader is active and, if not, will begin the 
		leadership acquisition process.
		'''
	...
	def acquire_leadership(self):
		'''
		Initiates the leadership acquisition process if the current 
		leader appears to have failed.
		'''
	...
	def recv_promise(self, acceptor_uid, proposal_id,
		...
		if not pre_leader and self.leader:
			...
			self.messenger.on_leadership_change(
								old_leader_uid, self.node_uid)
								
	def recv_prepare_nack(self, from_uid, proposal_id, promised_id):
		super(HeartbeatNode, self).recv_prepare_nack(from_uid,
