...
classes implement only the essential Paxos components and omit the
practical considerations (such as durability, message retransmissions,
NACKs, etc).
'''

import collections

# In order for the Paxos algorithm to function, all proposal ids must
...
# from the collections module whic allows us to write "proposal_id
# .number" instead of "proposal_id[0]".
...
class Messenger(object):
	...
	def send_accept(self, proposal_id, accepted_value):
		'''
	...
	def on_resolution(self, proposal_id, values):
		'''
...
class Proposer (object):
	...
	def set_proposal(self, value):
		'''
		Sets the proposal value for this node iff this node is not 
		already aware of another proposal having already been accepted.
		'''
	...
	def recv_promise(self, from_uid, proposal_id, prev_accepted_id, prev_accepted_value):
		'''
		Called when a Promise message is recevied from an Acceptor
		'''
...
class Learner(object):
	...
	def recv_accepted(self, from_uid, promised_id, accepted_value):
		...
		assert accepted_value == t[2], 'Value mismatch fro single proposal!'
		t[0] += 1
		t[1] += 1
