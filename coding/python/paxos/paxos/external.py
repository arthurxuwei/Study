'''
This module extends practical.Node to support external failure 
detection.
'''
...
class ExternalMessenger(practical.Messenger):
	...
	def on_leadership_change(self, prev_leader_uid, new_leader_uid):
		'''
		Called when a change in leadership is detected. Either UID
		may be None.
		'''
...
class ExternalNode(practical.Node):
	...
	acquire leadership of the Paxos instance. Continual polling of the prepare()
	function and prevention of eternal leadership battles is also the responsiility
	of the caller.
	...
	def recv_promise(self, acceptor_uid, proposal_id, prev_proposal_id, prev_proposal_value):
		...
		super(ExternalNode, self).recv_promise(	acceptor_uid, proposal_id, 
