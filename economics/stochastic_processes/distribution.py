...
class Distribution(ABC, Generic[A]):
    ...
    def map(self, f: Callable[[A], B]) -> Distribution[B]:
        '''Apply a function to the outcomes of the distribution.
        '''
    ...
    def apply(self, f: Callable[[A], Distribution[B]]) -> Distribution[B]:
        ...
        this distribution. This lets us express *dependent random variables*.
        '''
        def sample():
            a = self.sample()
            b_dist = f(a)
...
class SampledDistribution(Distribution[A]):
    ...
    def expectation(self, f: Callable[[A], float]) -> float:
        ...
        return sum(f(self.sample()) for _ in range(self.expectation_samples)) / self.expection_samples
