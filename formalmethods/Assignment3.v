...
(** Observe that the type of [fold] is parameterized by _two_ type
    ...
    different? *)
(** if X is a array's index and Y is the element of the array.X and Y may
    be different and it's useful*)
(* ###################################################### *)
...
(** for any n,m if trur = beq_nat n m then n = m
    induction n;
    First, suppose n = 0.  true = beq_nat 0 m -> 0 = m
           induction m;
           suppose m = 0. true = beq_nat 0 0 -> 0 = 0
           suppose m = S m'. true = beq_nat 0 S m' -> 0 = S m'
                             true = false -> false
    Next, suppose n = S n'. true = beq_nat S n' m -> S n' = m
           suppose m = 0. true = beq_nat S n' 0 -> S n' = 0
                          true = false -> false
           suppose m = S m'. true = beq_nat S n' S m' -> S n' = S m'
 *)
...
(* FILL IN HERE *)
Check d (b a 5).
    Error: The term "b a 5" has type "mumble" while it is expected to have type
 "Type".
Check d mumble (b a 5). OK~
Check d bool (b a 5).OK~
Check e bool true.OK~
Check e mumble (b c 0).OK~
Check e bool (b c 0).
