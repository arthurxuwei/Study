'''
This module builds upon the essential Paxos implementation 
and adds functionality required for most pratical uses of 
the algorithm.
'''
...
class Messenger (essential.Messenger):
	def send_prepare_nack(self, to_uid, proposal_id, promised_id):
		'''
		Sends a Prepar Nack message for the proposal to
		the specified node
		'''
		
	def send_accept_nack(self, to_uid, proposal_id, promised_id):
		'''
		Sends a Accept! Nack message for the proposal to 
		the specified node
		'''
		
	def on_leadership_acquired(self):
		'''
		Called when leadership has been aquired. This is 
		not a guaranteed position. Another node may have 
		successfully done so before this callback is exectued.
		Use this method with care.
		
		The safe way to guarantee leadership is to use a 
		full Paxos instance whith the resolution value being
		the UID of the leader node. To avoid potential issues
		arising from timing and/or failure, the election
		result may be restricted to a certain time window.
		Prior to the end of the window the leader may attempt
		to re-elect itself to extend it's term in office.
		'''
		
class Proposer (essential.Proposer):
	'''
	This class extends the functionality of the essential Proposer
	implementation by tracking whether the proposer believes itself
	to be the current leader of the Paxos instance. It also supports
	a flag to disable active paritcipation in the Paxos instance.
	
	The 'leader' attribute is a boolean value indicating the Proposer's
	belief in whether or not it is the leader. As the documentation
	for the Messenger.on_leadership_acquired() method describes,
	multiple nodes may simultaneously believe themselves to be the
	leader.
	
	The 'active' attribute is a boolean value indicating whether or 
	not the Proposer should send outgoing message(defaults to True).
	Setting this attribute to false places the Proposer in a "passive"
	mode where it processes all incoming messages but drops all messages
	...
	def set_proposal(self, value):
		'''
		Sets the proposal value for this node iff this node is not already
		aware of another proposal having already been accepted.
		'''
	...
	def prepare(self, increment_proposal_number=True):
		...
		proposal id. Otherwise the previously used proposal id will simply
		be retransmitted.
		'''
	...
	def recv_accept_nack(self, from_uid, proposal_id, promised_id):
		'''
		Called when an explicit NACK if sent is response to an accept message
		'''
		
	def resend_accept(self):
		'''
		Retransmits an Accept! message iff this node is the leader and has a
		proposal value
		'''
	...
	def recv_promise(self, 
		from_uid, proposal_id, prev_accepted_id, prev_accepted_value):
		'''
		Called when a Promise message is recvied from the network
		'''
		self.observe_proposal(from_uid, proposal_id)
		
		if self.leader \
			or proposal_id != self.proposal_id \
			or from_uid in self.promises_rcvd:
			return
		...
		if prev_accepted_id > self.last_accepted_id:
			...
			# If the Acceptor has already accepted a value,
			# we MUST set our proposal to that value.
			# otherwise, we may retain our current value.
...
class Acceptor (essential.Acceptor):
	'''
	Acceptors act as the fault-tolerant memory for Paxos. To ensure
	correctness in the presense of failure, Acceptors must be able 
	to remember the promises they've made even in the event of power
	outages. Consequently, any changes to the promised_id, accepted_id,
	and/or accepted_value must be persisted to stable media prior to 
	sending promise and accepted messages. After calling the recv_prepare()
	and recv_accept_request(), the property 'persistence_required' 
	should be checked to see if persistence is required.
	
	Note that because paxos permits any combination of dropped packets,
	not every promise/accepted message needs to be sent. This implementation
	only responses to the first prepare/accept_request message received and
	ignores all others until the Acceptor's values are persisted to stable
	media(which is typically a slow process). After saving the promised_id,
	accepted_id, and accepted_value variables, the 'persisted' method must
	be called to send the pending promise and/or accepted messages.
	
	The 'active' attribute is a boolean value indicating whether or not 
	the Acceptor shoulde send outgoing messages (defaults to True).
	Setting this attribute to false places the Acceptor in a 'passive' mode
	where it processes all incoming messages but drops all messages it
	...
	def persistance_required(self):
		return self.pending_promise is not None \
			or self.pending_accepted is not None
	...
	def recv_prepare(self, from_uid, proposal_id):
		...
		if proposal_id == self.promised_id:
			# Duplicate prepare message. No change in state in necessary
			# so the response may be sent immediately
			if self.active:
				self.messenger.send_promise(from_uid, 
					proposal_id, self.accepted_id, self.accepted_value)
	...
	def recv_accept_request(self, from_uid, proposal_id, value):
		...
		if proposal_id == self.accepted_id \
			...
			if self.active:
				self.messenger.send_accept_nack(from_uid,
									proposal_id, self.promised_id)
									
	def persisted(self):
		'''
		This method sends any pending Promise and/or Accepted messages.
		Prior to calling this method, the application must ensure that
...
class Learner (essential.Learner):
	'''
	This class extends the base in track which peers have accepted the
	final value. on_resolution() is still called only once. At the time
	of the call, the final_acceptors member variable will contain exactly
	quorum_size uids. Subsequent calls to recv_accepted will add the 
	uid of the sender if the accepted_value matches the final_value.
	'''
...
class Node(Proposer, Acceptor, Learner):
	'''
	This calss supports the common model where each node on a network 
	preforms all three Paxos roles, Proposer, Acceptor, and Learner.
	'''
