...
class PracticalProposerTests(test_essential.EssentialProposerTests):
	...
	def test_set_propsal_no_previous_value_as_leader(self):
		self.set_leader()
	...
	def test_set_propsal_no_previous_value_as_leader_not_active(self):
		self.set_leader()
		self.p.active = False
		self.p.set_proposal('foo')
		self.ae(self.p.proposed_value, 'foo')
	...
	def test_prepare_no_increment(self):
		self.p.prepare()
		self.am('prepare', PID(1, 'A'))
	...
	def test_pbserve_proposal(self):
		self.p.prepare()
		self.am('prepare', PID(1, 'A'))
...
class PracticalAcceptorTests(test_essential.EssentialAcceptorTests):
	...
	def test_recv_accept_request_less_than_promised(self):
		super(PracticalAcceptorTests, self) \
			.test_recv_accept_request_less_than_promised()
