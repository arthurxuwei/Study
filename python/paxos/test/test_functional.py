...
class HeartbeatTests(object):
	...
	def test_initial_leader(self):
		...
		#test_initial_wait() shows that the next call to p()
		#will result in the node attempting to assume leadership
		#by generating a new proposal_id. Reception of this 
		#heartbeat should reset the liveness timer and suppress
		#the coup attempt.
	...
	def test_regain_leader(self):
		...
		# The 6 here comes form using PID(5, 'B') as the heartbeat
		# proposal id the caused the loss of leadership
...
class HeartbeatProposerTester(test_practical.PracticalProposerTests,
					HeartbeatMessenger, unittest.TestCase):
